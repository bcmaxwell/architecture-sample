...
shown as dependencies rather than request flow. The trust boundary sits at the API
...
identity called out. The current-state note names what is not yet automated — trigger
...
volume and moved to a smaller tier. The slide states what was given up rather than only
what was saved, since the tradeoff is the part worth reviewing.
...
PowerPoint. Every diagram is built from native vector shapes rather than imported
images, so shapes, text, and color can be edited directly.

**Design notes.** Meaning is carried by fill contrast (dark versus light) rather than
